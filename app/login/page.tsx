'use client';

import { authClient } from '@/lib/auth-client';

export default function LoginPage() {
  const handleGoogleLogin = () => {
    authClient.signIn.social({ provider: 'google', callbackURL: '/' });
  };

  return (
    <div
      className="min-h-screen flex items-center justify-center"
      style={{ backgroundColor: '#f0ece4' }}
    >
      <div
        className="flex flex-col items-center gap-8 p-12 rounded-2xl"
        style={{
          backgroundColor: '#faf8f3',
          boxShadow: '0 8px 32px rgba(0,0,0,0.12)',
          border: '1px solid rgba(0,0,0,0.06)',
        }}
      >
        <div className="flex flex-col items-center gap-2">
          <h1 className="text-3xl font-serif text-stone-800 tracking-wide">My Journals</h1>
          <p className="text-sm text-stone-400">Sign in to access your journals</p>
        </div>

        <button
          onClick={handleGoogleLogin}
          className="flex items-center gap-3 px-6 py-3 rounded-lg text-sm font-medium
            text-stone-700 transition-all hover:bg-stone-100 active:scale-95"
          style={{
            border: '1px solid rgba(0,0,0,0.12)',
            backgroundColor: '#fff',
          }}
        >
          <svg width="18" height="18" viewBox="0 0 18 18">
            <path fill="#4285F4" d="M16.51 8H8.98v3h4.3c-.18 1-.74 1.48-1.6 2.04v2.01h2.6a7.8 7.8 0 0 0 2.38-5.88c0-.57-.05-.66-.15-1.18"/>
            <path fill="#34A853" d="M8.98 17c2.16 0 3.97-.72 5.3-1.94l-2.6-2a4.8 4.8 0 0 1-7.18-2.54H1.83v2.07A8 8 0 0 0 8.98 17"/>
            <path fill="#FBBC05" d="M4.5 10.52a4.8 4.8 0 0 1 0-3.04V5.41H1.83a8 8 0 0 0 0 7.18z"/>
            <path fill="#EA4335" d="M8.98 4.18c1.17 0 2.23.4 3.06 1.2l2.3-2.3A8 8 0 0 0 1.83 5.4L4.5 7.49a4.77 4.77 0 0 1 4.48-3.31"/>
          </svg>
          Continue with Google
        </button>
      </div>
    </div>
  );
}
